...
func mapSchemaError(opID, method, path string, status int, contentType string, err error) []model.Finding {
	...
	if ve, ok := err.(*jsonschema.ValidationError); ok {
		...
		if len(out) == 0 {
			out = append(out, model.Finding{
				OpID: opID, Method: strings.ToUpper(method), Path: path, Status: status, ContentType: contentType,
				Kind: model.KindSchemaViolation, Severity: model.SevHigh,
				JSONPath: "$", Message: ve.Message,
			})
		}
		return out
	}
	out = append(out, model.Finding{
		OpID: opID, Method: strings.ToUpper(method), Path: path, Status: status, ContentType: contentType,
		Kind: model.KindSchemaViolation, Severity: model.SevHigh,
		JSONPath: "$", Message: err.Error(),
	})
	return out
}

func flattenValidationError(opID, method, path string, status int, contentType string, ve *jsonschema.ValidationError, out *[]model.Finding) {
	...
	*out = append(*out, model.Finding{
	...
	for _, c := range ve.Causes {
		if cv, ok := c.(*jsonschema.ValidationError); ok {
			flattenValidationError(opID, method, path, status, contentType, cv, out)
		}
	}
}

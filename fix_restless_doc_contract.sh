...
log "Fixer doc/test-kontrakt: legacy docs skal ikke telle som publiserte løfter"

mkdir -p docs/internal archive/legacy

# 1) Marker legacy tydelig
cat > archive/legacy/README.md <<'MD'
# Legacy archive

This folder contains historical design notes, old CLI sketches, and abandoned/experimental documentation.

Files under `archive/legacy/` are **not part of the current public Restless CLI contract** and must not be used as release documentation or command-test input.

Current public documentation lives in:

- `README.md`
- `docs/`
- command output from `restless --help`
MD

# 2) Legg inn advarsel i gamle legacy-docs hvis mangler
for f in archive/legacy/docs/*.md; do
  [ -f "$f" ] || continue
  if ! grep -q "LEGACY / HISTORICAL" "$f"; then
    tmp="$(mktemp)"
    cat > "$tmp" <<'MD'
> **LEGACY / HISTORICAL**
>
...
# 3) Lag/erstatt skikkelig doc stress-script som ignorerer legacy/scratch/dev rot
cat > restless_doc_stress.sh <<'TEST'
#!/usr/bin/env bash
set -u
...
chmod +x restless_doc_stress.sh

# 4) Profesjonell ignore for testartefakter
touch .gitignore
grep -qxF 'doc-stress-*/' .gitignore || echo 'doc-stress-*/' >> .gitignore
grep -qxF 'test_logs/' .gitignore || echo 'test_logs/' >> .gitignore

# 5) Kjør test
TIMEOUT="${TIMEOUT:-20}" ./restless_doc_stress.sh

echo
echo "[*] Git diff:"
git diff -- archive/legacy/README.md archive/legacy/docs .gitignore restless_doc_stress.sh | sed -n '1,260p'
